...
import { Bot, Expand, Keyboard, Loader2, Minimize2, MousePointer2, Play, UserRound, X } from "lucide-react";
...
export function BrowserSessionCard({ sessionId }: { sessionId: string }) {
  ...
  const refreshState = useCallback(async () => {
    const res = await fetch(`/api/apply/browser/state?sessionId=${encodeURIComponent(sessionId)}`, { cache: "no-store" });
    if (!res.ok) {
      setClosed(true);
      return;
    }
  ...
  const returnToAgent = async () => {
    try {
      ...
      if (apply.sessionId === sessionId) await apply.resumeAgent();
      await refreshState();
    } catch (e) {
  ...
  const close = async () => {
    ...
    if (apply.sessionId === sessionId) apply.reset();
    setClosed(true);
    setExpanded(false);
  };
  ...
  const frame = (large: boolean) => (
    <div className={cn("relative overflow-hidden bg-black", large ? "h-[calc(100vh-8rem)]" : "aspect-[16/10]") }>
      {!closed && (
        <img
          ref={frameRef}
          src={`/api/apply/browser/frame?sessionId=${encodeURIComponent(sessionId)}&n=${frameNonce}`}
      ...
      {!state && !closed && <Loader2 className="absolute left-1/2 top-1/2 size-5 -translate-x-1/2 -translate-y-1/2 animate-spin text-white/70" />}
  ...
  const toolbar = (large: boolean) => (
    <div className="flex items-center gap-2 border-b border-border bg-surface px-3 py-2">
      ...
      <button onClick={() => setExpanded(!large)} className="rounded-md p-1 text-faint hover:bg-surface-hover hover:text-foreground" aria-label={large ? "Exit fullscreen" : "Expand browser"}>{large ? <Minimize2 className="size-3.5" /> : <Expand className="size-3.5" />}</button>
    ...
    <>
      <div className="overflow-hidden rounded-xl border border-border bg-surface/60 shadow-sm">
        ...
        <div className="flex items-center gap-1.5 border-t border-border px-3 py-1.5 text-[10px] text-faint">
          ...
          {apply.status !== "idle" && apply.sessionId === sessionId && <span className="ml-auto">{apply.status}</span>}
        </div>
