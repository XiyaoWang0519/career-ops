...
const CANON_STATUS = ["Evaluated", "Pursuing", "Applied", "Responded", "Interview", "Offer", "Hired", "Rejected", "Discarded", "SKIP"];

const TAB_VALUES = [
  "INBOX", "REVIEW", "ACTIVE", "CLOSED", "ALL", "EVALUATED", "PURSUING", "APPLIED", "RESPONDED", "INTERVIEW", "OFFER", "HIRED", "REJECTED", "DISCARDED", "SKIP",
] as const;
...
function isAllowedPath(p: string): boolean {
  ...
  if (path === "/") return true;
  return /^\/(chat|explore|pipeline|portals|analytics|cv|config|apply|jobs|followups)(\/[^/]+)?$/.test(path);
}
